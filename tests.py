...
class TestRelativeDeltaOutput(unittest.TestCase):
    ...
    def test_timeparse_multipliers(self):
        """Test parsing time unit multipliers."""
        self.assertEqual(timeparse.parse('32 min', as_timedelta=True),
                         relativedelta(seconds=1920))
        self.assertEqual(timeparse.parse('1 min', as_timedelta=True),
                         relativedelta(seconds=60))
        self.assertEqual(timeparse.parse('1 hours', as_timedelta=True),
                         relativedelta(seconds=3600))
        self.assertEqual(timeparse.parse('1 day', as_timedelta=True),
                         relativedelta(seconds=86400))
        self.assertEqual(timeparse.parse('1 sec', as_timedelta=True),
                         relativedelta(seconds=1))

    def test_timeparse_signs(self):
        """Test parsing time signs."""
        self.assertEqual(timeparse.parse('+32 m 1 s', as_timedelta=True), relativedelta(seconds = 1921))
        self.assertEqual(timeparse.parse('+ 32 m 1 s', as_timedelta=True), relativedelta(seconds = 1921))
        self.assertEqual(timeparse.parse('-32 m 1 s', as_timedelta=True), relativedelta(seconds = -1921))
        self.assertEqual(timeparse.parse('- 32 m 1 s', as_timedelta=True), relativedelta(seconds = -1921))
    ...
    def test_timeparse_1(self):
        """timeparse test case 1."""
        self.assertEqual(timeparse.parse('32m', as_timedelta=True), relativedelta(seconds = 1920))
        self.assertEqual(timeparse.parse('+32m', as_timedelta=True), relativedelta(seconds = 1920))
        self.assertEqual(timeparse.parse('-32m', as_timedelta=True), relativedelta(seconds = -1920))

    def test_timeparse_2(self):
        """timeparse test case 2."""
        self.assertEqual(timeparse.parse('2h32m', as_timedelta=True), relativedelta(seconds = 9120))
        self.assertEqual(timeparse.parse('+2h32m', as_timedelta=True), relativedelta(seconds = 9120))
        self.assertEqual(timeparse.parse('-2h32m', as_timedelta=True), relativedelta(seconds = -9120))

    def test_timeparse_3(self):
        """timeparse test case 3."""
        self.assertEqual(timeparse.parse('3d2h32m', as_timedelta=True), relativedelta(seconds = 268320))
        self.assertEqual(timeparse.parse('+3d2h32m', as_timedelta=True), relativedelta(seconds = 268320))
        self.assertEqual(timeparse.parse('-3d2h32m', as_timedelta=True), relativedelta(seconds = -268320))

    def test_timeparse_4(self):
        """timeparse test case 4."""
        self.assertEqual(timeparse.parse('1w3d2h32m', as_timedelta=True), relativedelta(seconds = 873120))
        self.assertEqual(timeparse.parse('+1w3d2h32m', as_timedelta=True), relativedelta(seconds = 873120))
        self.assertEqual(timeparse.parse('-1w3d2h32m', as_timedelta=True), relativedelta(seconds = -873120))

    def test_timeparse_5(self):
        """timeparse test case 5."""
        self.assertEqual(timeparse.parse('1w 3d 2h 32m', as_timedelta=True), relativedelta(seconds = 873120))
        self.assertEqual(timeparse.parse('+1w 3d 2h 32m', as_timedelta=True), relativedelta(seconds = 873120))
        self.assertEqual(timeparse.parse('-1w 3d 2h 32m', as_timedelta=True), relativedelta(seconds = -873120))

    def test_timeparse_6(self):
        """timeparse test case 6."""
        self.assertEqual(timeparse.parse('1 w 3 d 2 h 32 m', as_timedelta=True), relativedelta(seconds = 873120))
        self.assertEqual(timeparse.parse('+1 w 3 d 2 h 32 m', as_timedelta=True), relativedelta(seconds = 873120))
        self.assertEqual(timeparse.parse('-1 w 3 d 2 h 32 m', as_timedelta=True), relativedelta(seconds = -873120))

    def test_timeparse_7(self):
        """timeparse test case 7."""
        self.assertEqual(timeparse.parse('4:13', as_timedelta=True), relativedelta(seconds = 253))
        self.assertEqual(timeparse.parse('+4:13', as_timedelta=True), relativedelta(seconds = 253))
        self.assertEqual(timeparse.parse('-4:13', as_timedelta=True), relativedelta(seconds = -253))
    ...
    def test_timeparse_8(self):
        """timeparse test case 8."""
        self.assertEqual(timeparse.parse('4:13:02', as_timedelta=True), relativedelta(seconds = 15182))
        self.assertEqual(timeparse.parse('+4:13:02', as_timedelta=True), relativedelta(seconds = 15182))
        self.assertEqual(timeparse.parse('-4:13:02', as_timedelta=True), relativedelta(seconds = -15182))

    def test_timeparse_9(self):
        """timeparse test case 9."""
        self.assertEqual(timeparse.parse('4:13:02.266', as_timedelta=True), relativedelta(seconds = 15182, microseconds=266 * 1000))
        self.assertEqual(timeparse.parse('+4:13:02.266', as_timedelta=True), relativedelta(seconds = 15182, microseconds=266 * 1000))
        self.assertEqual(timeparse.parse('-4:13:02.266', as_timedelta=True), relativedelta(seconds = -15182, microseconds=-266 * 1000))

    def test_timeparse_10(self):
        """timeparse test case 10."""
        self.assertEqual(timeparse.parse('2:04:13:02.266', as_timedelta=True),
                               relativedelta(seconds = 187982, microsecond = 266 * 1000))
        self.assertEqual(timeparse.parse('+2:04:13:02.266', as_timedelta=True),
                               relativedelta(seconds = 187982, microsecond = 266 * 1000))
        self.assertEqual(timeparse.parse('-2:04:13:02.266', as_timedelta=True),
                               relativedelta(seconds = -187982, microsecond = - 266 * 1000))
    ...
    def test_timeparse_granularity_1(self):
        """Check that minute-level granularity applies correctly."""
        self.assertEqual(timeparse.parse('4:32', granularity='minutes', as_timedelta=True), relativedelta(seconds = 272*60))
        self.assertEqual(timeparse.parse('+4:32', granularity='minutes', as_timedelta=True), relativedelta(seconds = 272*60))
        self.assertEqual(timeparse.parse('-4:32', granularity='minutes', as_timedelta=True), relativedelta(seconds = -272*60))
    ...
    def test_timeparse_11(self):
        """timeparse test case 11."""
        # uptime format
        self.assertEqual(timeparse.parse('2 days,  4:13:02', as_timedelta=True), relativedelta(seconds = 187982))
        self.assertEqual(timeparse.parse('+2 days,  4:13:02', as_timedelta=True), relativedelta(seconds = 187982))
        self.assertEqual(timeparse.parse('-2 days,  4:13:02', as_timedelta=True), relativedelta(seconds = -187982))

    def test_timeparse_12(self):
        """timeparse test case 12."""
        self.assertEqual(timeparse.parse('2 days,  4:13:02.266', as_timedelta=True),
                               relativedelta(seconds = 187982, microsecond = 1000 * 266))
        self.assertEqual(timeparse.parse('+2 days,  4:13:02.266', as_timedelta=True),
                               relativedelta(seconds = 187982, microsecond = 1000 * 266))
        self.assertEqual(timeparse.parse('-2 days,  4:13:02.266', as_timedelta=True),
                               relativedelta(seconds = -187982, microsecond = -1000 * 266))
    ...
    def test_timeparse_13(self):
        """timeparse test case 13."""
        self.assertEqual(timeparse.parse('5hr34m56s', as_timedelta=True), relativedelta(seconds = 20096))
        self.assertEqual(timeparse.parse('+5hr34m56s', as_timedelta=True), relativedelta(seconds = 20096))
        self.assertEqual(timeparse.parse('-5hr34m56s', as_timedelta=True), relativedelta(seconds = -20096))

    def test_timeparse_14(self):
        """timeparse test case 14."""
        self.assertEqual(timeparse.parse('5 hours, 34 minutes, 56 seconds', as_timedelta=True),
                         relativedelta(seconds = 20096))
        self.assertEqual(timeparse.parse('+5 hours, 34 minutes, 56 seconds', as_timedelta=True),
                         relativedelta(seconds = 20096))
        self.assertEqual(timeparse.parse('-5 hours, 34 minutes, 56 seconds', as_timedelta=True),
                         relativedelta(seconds = -20096))
    ...
    def test_timeparse_15(self):
        """timeparse test case 15."""
        self.assertEqual(timeparse.parse('5 hrs, 34 mins, 56 secs', as_timedelta=True), relativedelta(seconds = 20096))
        self.assertEqual(timeparse.parse('+5 hrs, 34 mins, 56 secs', as_timedelta=True), relativedelta(seconds = 20096))
        self.assertEqual(timeparse.parse('-5 hrs, 34 mins, 56 secs', as_timedelta=True), relativedelta(seconds = -20096))
    ...
    def test_timeparse_16(self):
        """timeparse test case 16."""
        self.assertEqual(
            timeparse.parse('2 days, 5 hours, 34 minutes, 56 seconds', as_timedelta=True),
            relativedelta(seconds = 192896))
        self.assertEqual(
            timeparse.parse('+2 days, 5 hours, 34 minutes, 56 seconds', as_timedelta=True),
            relativedelta(seconds = 192896))
        self.assertEqual(
    ...
    def test_timeparse_17(self):
        """timeparse test case 17."""
        self.assertEqual(timeparse.parse('1.2 m', as_timedelta=True), relativedelta(seconds = 72))
        self.assertEqual(timeparse.parse('+1.2 m', as_timedelta=True), relativedelta(seconds = 72))
        self.assertEqual(timeparse.parse('-1.2 m', as_timedelta=True), -relativedelta(seconds = 72))

    def test_timeparse_18(self):
        """timeparse test case 18."""
        self.assertEqual(timeparse.parse('1.2 min', as_timedelta=True), relativedelta(seconds = 72))
        self.assertEqual(timeparse.parse('+1.2 min', as_timedelta=True), relativedelta(seconds = 72))
        self.assertEqual(timeparse.parse('-1.2 min', as_timedelta=True), relativedelta(seconds = -72))

    def test_timeparse_19(self):
        """timeparse test case 19."""
        self.assertEqual(timeparse.parse('1.2 mins', as_timedelta=True), relativedelta(seconds = 72))
        self.assertEqual(timeparse.parse('+1.2 mins', as_timedelta=True), relativedelta(seconds = 72))
        self.assertEqual(timeparse.parse('-1.2 mins', as_timedelta=True), relativedelta(seconds = -72))

    def test_timeparse_20(self):
        """timeparse test case 20."""
        self.assertEqual(timeparse.parse('1.2 minute', as_timedelta=True), relativedelta(seconds = 72))
        self.assertEqual(timeparse.parse('+1.2 minute', as_timedelta=True), relativedelta(seconds = 72))
        self.assertEqual(timeparse.parse('-1.2 minute', as_timedelta=True), relativedelta(seconds = -72))

    def test_timeparse_21(self):
        """timeparse test case 21."""
        self.assertEqual(timeparse.parse('1.2 minutes', as_timedelta=True), relativedelta(seconds = 72))
        self.assertEqual(timeparse.parse('+1.2 minutes', as_timedelta=True), relativedelta(seconds = 72))
        self.assertEqual(timeparse.parse('-1.2 minutes', as_timedelta=True), relativedelta(seconds = -72))

    def test_timeparse_22(self):
        """timeparse test case 22."""
        self.assertEqual(timeparse.parse('172 hours', as_timedelta=True), relativedelta(seconds = 619200))
        self.assertEqual(timeparse.parse('+172 hours', as_timedelta=True), relativedelta(seconds = 619200))
        self.assertEqual(timeparse.parse('-172 hours', as_timedelta=True), relativedelta(seconds = -619200))

    def test_timeparse_23(self):
        """timeparse test case 23."""
        self.assertEqual(timeparse.parse('172 hr', as_timedelta=True), relativedelta(seconds = 619200))
        self.assertEqual(timeparse.parse('+172 hr', as_timedelta=True), relativedelta(seconds = 619200))
        self.assertEqual(timeparse.parse('-172 hr', as_timedelta=True), relativedelta(seconds = -619200))

    def test_timeparse_24(self):
        """timeparse test case 24."""
        self.assertEqual(timeparse.parse('172 h', as_timedelta=True), relativedelta(seconds = 619200))
        self.assertEqual(timeparse.parse('+172 h', as_timedelta=True), relativedelta(seconds = 619200))
        self.assertEqual(timeparse.parse('-172 h', as_timedelta=True), relativedelta(seconds = -619200))

    def test_timeparse_25(self):
        """timeparse test case 25."""
        self.assertEqual(timeparse.parse('172 hrs', as_timedelta=True), relativedelta(seconds = 619200))
        self.assertEqual(timeparse.parse('+172 hrs', as_timedelta=True), relativedelta(seconds = 619200))
        self.assertEqual(timeparse.parse('-172 hrs', as_timedelta=True), relativedelta(seconds = -619200))

    def test_timeparse_26(self):
        """timeparse test case 26."""
        self.assertEqual(timeparse.parse('172 hour', as_timedelta=True), relativedelta(seconds = 619200))
        self.assertEqual(timeparse.parse('+172 hour', as_timedelta=True), relativedelta(seconds = 619200))
        self.assertEqual(timeparse.parse('-172 hour', as_timedelta=True), relativedelta(seconds = -619200))

    def test_timeparse_27(self):
        """timeparse test case 27."""
        self.assertEqual(timeparse.parse('1.24 days', as_timedelta=True), relativedelta(seconds = 107136))
        self.assertEqual(timeparse.parse('+1.24 days', as_timedelta=True), relativedelta(seconds = 107136))
        self.assertEqual(timeparse.parse('-1.24 days', as_timedelta=True), relativedelta(seconds = -107136))

    def test_timeparse_28(self):
        """timeparse test case 28."""
        self.assertEqual(timeparse.parse('5 d', as_timedelta=True), relativedelta(seconds = 432000))
        self.assertEqual(timeparse.parse('+5 d', as_timedelta=True), relativedelta(seconds = 432000))
        self.assertEqual(timeparse.parse('-5 d', as_timedelta=True), relativedelta(seconds = -432000))

    def test_timeparse_29(self):
        """timeparse test case 29."""
        self.assertEqual(timeparse.parse('5 day', as_timedelta=True), relativedelta(seconds = 432000))
        self.assertEqual(timeparse.parse('+5 day', as_timedelta=True), relativedelta(seconds = 432000))
        self.assertEqual(timeparse.parse('-5 day', as_timedelta=True), relativedelta(seconds = -432000))

    def test_timeparse_30(self):
        """timeparse test case 30."""
        self.assertEqual(timeparse.parse('5 days', as_timedelta=True), relativedelta(seconds = 432000))
        self.assertEqual(timeparse.parse('+5 days', as_timedelta=True), relativedelta(seconds = 432000))
        self.assertEqual(timeparse.parse('-5 days', as_timedelta=True), relativedelta(seconds = -432000))

    def test_timeparse_31(self):
        """timeparse test case 31."""
        self.assertEqual(timeparse.parse('5.6 wk', as_timedelta=True), relativedelta(seconds = 3386880))
        self.assertEqual(timeparse.parse('+5.6 wk', as_timedelta=True), relativedelta(seconds = 3386880))
        self.assertEqual(timeparse.parse('-5.6 wk', as_timedelta=True), relativedelta(seconds = -3386880))

    def test_timeparse_32(self):
        """timeparse test case 32."""
        self.assertEqual(timeparse.parse('5.6 week', as_timedelta=True), relativedelta(seconds = 3386880))
        self.assertEqual(timeparse.parse('+5.6 week', as_timedelta=True), relativedelta(seconds = 3386880))
        self.assertEqual(timeparse.parse('-5.6 week', as_timedelta=True), relativedelta(seconds = -3386880))

    def test_timeparse_33(self):
        """timeparse test case 33."""
        self.assertEqual(timeparse.parse('5.6 weeks', as_timedelta=True), relativedelta(seconds = 3386880))
        self.assertEqual(timeparse.parse('+5.6 weeks', as_timedelta=True), relativedelta(seconds = 3386880))
        self.assertEqual(timeparse.parse('-5.6 weeks', as_timedelta=True), relativedelta(seconds = -3386880))
